...
last_strokes = defaultdict(lambda: 0)

for outline in main_dictionary:
    strokes = outline.split('/')
    last_stroke = strokes[-1]
    if last_stroke in affix_dict:
        last_strokes[last_stroke] += 1


for stroke, count in sorted(last_strokes.items(), reverse=True, key=lambda pair: pair[1]):
